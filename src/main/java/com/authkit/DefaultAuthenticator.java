...
public class DefaultAuthenticator implements Authenticator {
  ...
  private final Cache<String, AuthkitPrincipal> principalCache =
      Caffeine.newBuilder().maximumSize(10_000).expireAfterWrite(Duration.ofMinutes(120)).build();

  private final Cache<String, ParserAndUserinfoEndpoint> keyIdAndUserinfoCache =
      Caffeine.newBuilder().maximumSize(100).expireAfterWrite(Duration.ofMinutes(120)).build();
  ...
  private Mono<ParserAndUserinfoEndpoint> getParserAndUserinfo(String keyId) {
    return CacheMono.lookup(
            k -> Mono.justOrEmpty(keyIdAndUserinfoCache.getIfPresent(k)).map(Signal::next), keyId)
                ...
                client
                    ...
                    .responseSingle(
                        (r, b) -> {
                          if (r.status().code() == 200) {
                            return b.asInputStream()
                                .map(i -> new Tuple<>(i, oidc.userinfoEndpoint));
                          } else {
                            // TODO - Want some better logging here
                            throw new AuthkitException(
                                "unable to load jwks, code: " + r.status().code());
                          }
                        })
                    .map(
                        i -> {
        ...
        .map(
            pk -> {
              JwtParserBuilder builder =
                  Jwts.parserBuilder().setSigningKey(pk.value1).requireIssuer(issuer);
  ...
  public Publisher<AuthkitPrincipal> doAuthenticate(final String token) {
    ...
    return Mono.fromCallable(() -> validateAndGetKeyId(token))
        .flatMap(this::getParserAndUserinfo)
        .map(
            j -> {
                    ...
                    .responseSingle(
                        (r, b) -> {
                          if (r.status().code() == 200) {
                            ...
                            // TODO - Want some better logging here
                            throw new AuthkitException(
                                "unable to load userinfo, code: " + r.status().code());
                          }
                        }))
        .map(
            t -> {
